...
const character = (state = initialState, action = {}) => {
  switch (action.type) {
    ...
    case SPEND_RESOURCES_FOR_CRAFT:
      ...
      craftedEquipments.forEach((elem) => {
        if (elem.name == action.recipe.type) {
          elem.quantity += 1;
          if (
            elem.reserve.find((weapon) => weapon.item_id == action.recipe.id) ==
            undefined
          ) {
            let crafted = {
              ...
              img_path: action.recipe.name
                .replace(/['"]+/g, "")
                .replace(/\s/g, ""),
            };
            if (elem.name == "arme") {
              crafted.statistique = action.recipe.attribute.filter(
                (att) => att.name == "force"
              );
            } else if (elem.name == "armure") {
              crafted.statistique = action.recipe.attribute.filter(
                (att) => att.name == "endurance"
              );
    ...
    case SET_CHARACTER_DATA:
      ...
      const dataForce = action.data.attributes.find(
        (obj) => obj.name == "force"
      );
      const dataEndurance = action.data.attributes.find(
        (obj) => obj.name == "endurance"
      );
      const dataDextérité = action.data.attributes.find(
        (obj) => obj.name == "dextérité"
      );
      const dataPoints = action.data.attributes.find(
        (obj) => obj.name == "points de caractéristiques"
      );
      const dataVie = action.data.attributes.find(
        (obj) => obj.name == "points de vie"
      );
      return {
        ...state,
    ...
    case SET_DETAILS:
      const { item_id, name, img_path, description, type, statistique } =
        action.detailsObj;
      let quantity;
